...
int main(void) {
    ...
    const int scrollCases[][5] = {
        { -16, 800, 3, 266, 112 }, { 0, 800, 2, 266, 112 },   { 400, 800, 3, 266, 562 },
        { 400, 800, 2, 266, 612 }, { 800, 800, 2, 266, 758 }, { 800, 800, 2, 240, 768 },
    };
    for (unsigned i = 0; i < sizeof(scrollCases) / sizeof(scrollCases[0]); ++i) {
        const int* c = scrollCases[i];
        if (Port_HorizontalMinishPathScroll(c[0], c[1], c[2], c[3]) != c[4]) {
            fprintf(stderr, "FAIL: parallax clamp case %u\n", i);
            ++failures;
        }
    }
    ...
    for (u32 layer = 0; layer < 2; ++layer) {
        for (u32 page = 0; page <= 48; ++page) {
            ...
            for (u32 row = 0; row < 32; ++row) {
                if (memcmp(actual + row * 32u, gUnk_02006F00 + layer * 0x2000u + page * 4u + row * 0x100u, 64) != 0) {
    ...
    }
    {
        ...
        sub_08058004(49u * 16u, gUnk_02006F00, actual);
                    ...
                    if (output[dst] != gMapDataTopSpecial[src]) {
                        if (failures++ < 8u) {
                            fprintf(stderr, "FAIL: layer %u destination 0x%X got 0x%04X expected 0x%04X\n", layer, dst,
                                    output[dst], gMapDataTopSpecial[src]);
                        }
                    }
                }
            }
